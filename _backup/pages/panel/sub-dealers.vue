<template>
  <div class="space-y-4">
    <UCard>
      <template #header>
        <h1 class="text-xl font-bold">{{ t('dealers.dealer_list') }}</h1>
      </template>
    ...
    <div class="flex flex-wrap items-center gap-1.5">
      ...
      <UButton icon="i-lucide-minus-circle" variant="outline" color="neutral" size="sm" :disabled="selectedRows.length === 0">
        {{ t('dealers.all_player_balance_to_zero') }}
      </UButton>
    ...
    <div class="flex items-center justify-between gap-4">
      <UInput v-model="searchQuery" icon="i-lucide-search" :placeholder="t('dealers.search_dealers')" class="max-w-xs" />
    ...
    <div class="border border-gray-200 dark:border-gray-800 rounded-lg overflow-hidden">
      <UTable :data="paginatedData" :columns="filteredColumns" :loading="status === 'pending'">
        ...
        <template #totalBalance-cell="{ row }">{{ formatCurrency(row.original.totalBalance) }}</template>
...
const columns = [
  ...
  { accessorKey: 'username', header: t('dealers.dealer') },
  { accessorKey: 'balance', header: t('common.balance') },
  { accessorKey: 'totalBalance', header: t('dealers.total_balance') },
  { accessorKey: 'status', header: t('common.status') },
  { accessorKey: 'lastLoginIp', header: t('common.last_login_ip') },
  { accessorKey: 'lastLogin', header: t('common.last_login') },
  { accessorKey: 'createdAt', header: t('dealers.membership_date') }
]

const { sorts, sortBy, sortDirection, addSort, removeSort, updateSortColumn, updateSortDirection, clearSorts, availableSortColumns, visibleColumnKeys, toggleColumn, filteredColumns, sortableColumns, toggleableColumns } = useTableStore('sub-dealers', columns, 'createdAt')

const queryParams = computed(() => ({
  role: 'SUB_DEALER', page: currentPage.value, limit: pageSize.value,
  sortBy: sortBy.value, sortDirection: sortDirection.value,
  ...(searchQuery.value.trim() ? { search: searchQuery.value.trim() } : {})
}))

const { data: sdData, refresh, status } = await useAsyncData(
  'sub-dealers-list',
  () => $fetch<{ data: any[], total: number }>('/api/users', { query: queryParams.value }),
  { watch: [queryParams] }
)

const allSubDealers = computed(() => (sdData.value?.data ?? []).map(sd => ({
  ...sd, status: sd.status ? sd.status.toLowerCase() : '',
  adminUsername: sd.adminUsername ?? '-', dealerUsername: sd.dealerUsername ?? '-',
  balance: sd.balance ?? 0, totalBalance: sd.totalBalance ?? 0, lastLogin: sd.lastLogin ?? null, lastLoginIp: sd.lastLoginIp ?? '-'
})))

const filteredData = computed(() => allSubDealers.value)
const totalPages = computed(() => Math.max(1, Math.ceil((sdData.value?.total ?? 0) / pageSize.value)))
const paginatedData = computed(() => allSubDealers.value)
const selectedRows = computed(() => allSubDealers.value.filter(sd => selectedIds.value.has(sd.id)))
const editUser = computed(() => selectedRows.value.length === 1 ? selectedRows.value[0] : null)
const allSelected = computed(() => paginatedData.value.length > 0 && paginatedData.value.every(sd => selectedIds.value.has(sd.id)))
const someSelected = computed(() => !allSelected.value && paginatedData.value.some(sd => selectedIds.value.has(sd.id)))

function toggleAll() { if (allSelected.value) paginatedData.value.forEach(sd => selectedIds.value.delete(sd.id)); else paginatedData.value.forEach(sd => selectedIds.value.add(sd.id)) }
