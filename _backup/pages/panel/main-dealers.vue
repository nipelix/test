<template>
  <div class="space-y-4">
    ...
    <AdminCreditModal v-model:open="creditModalOpen" :all-users="allDealers" :pre-selected-ids="selectedRows.map(r => r.id)" :label="t('dealers.dealer')" @success="handleModalSuccess" />
...
const columns = [
  ...
  { accessorKey: 'agentUsername', header: t('dashboard.agent') },
...
const { sorts, sortBy, sortDirection, addSort, removeSort, updateSortColumn, updateSortDirection, clearSorts, availableSortColumns, visibleColumnKeys, toggleColumn, filteredColumns, sortableColumns, toggleableColumns } = useTableStore('main-dealers', columns, 'createdAt')

const queryParams = computed(() => ({
  role: 'DEALER', page: currentPage.value, limit: pageSize.value,
  sortBy: sortBy.value, sortDirection: sortDirection.value,
  ...(searchQuery.value.trim() ? { search: searchQuery.value.trim() } : {})
}))

const { data: dealersData, refresh, status } = await useAsyncData(
  'main-dealers-list',
  () => $fetch<{ data: any[], total: number }>('/api/users', { query: queryParams.value }),
  { watch: [queryParams] }
)

const allDealers = computed(() => (dealersData.value?.data ?? []).map(d => ({
  ...d, status: d.status ? d.status.toLowerCase() : '',
  adminUsername: d.adminUsername ?? '-', agentUsername: d.agentUsername ?? '-',
  credit: d.credit ?? 0, lastLogin: d.lastLogin ?? null, lastLoginIp: d.lastLoginIp ?? '-'
})))

const filteredData = computed(() => allDealers.value)
const totalPages = computed(() => Math.max(1, Math.ceil((dealersData.value?.total ?? 0) / pageSize.value)))
const paginatedData = computed(() => allDealers.value)
const selectedRows = computed(() => allDealers.value.filter(d => selectedIds.value.has(d.id)))
const editUser = computed(() => selectedRows.value.length === 1 ? selectedRows.value[0] : null)
const allSelected = computed(() => paginatedData.value.length > 0 && paginatedData.value.every(d => selectedIds.value.has(d.id)))
const someSelected = computed(() => !allSelected.value && paginatedData.value.some(d => selectedIds.value.has(d.id)))

function toggleAll() { if (allSelected.value) paginatedData.value.forEach(d => selectedIds.value.delete(d.id)); else paginatedData.value.forEach(d => selectedIds.value.add(d.id)) }
