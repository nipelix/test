...
const creditWithdrawSchema = z.object({
...
export default defineEventHandler(async (event) => {
  ...
  const body = await readValidatedBody(event, creditWithdrawSchema.parse)
  ...
  if (body.amount > currentCredit) {
    throw createError({ statusCode: 400, statusMessage: 'Insufficient credit' })
  }
