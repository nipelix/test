...
import { sportProviderMappings } from '../../../database/schema'
...
export default defineEventHandler(async (event) => {
  ...
  const [updated] = await db.update(sportProviderMappings).set(body).where(eq(sportProviderMappings.id, id)).returning()
