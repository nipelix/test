<template>
  <div class="space-y-4">
    <UCard>
      ...
      <div class="space-y-4">
        <AdminEntityToolbar
          :search="searchQuery" :search-placeholder="t('countries.search')" :add-label="t('countries.add_country')"
    ...
    <USlideover v-model:open="formOpen" :title="editItem ? t('countries.edit_country') : t('countries.add_country')">
      <template #body>
        <form class="space-y-4" @submit.prevent="handleSubmit">
          ...
          <UFormField :label="t('countries.code')" required><UInput v-model="form.code" :placeholder="t('countries.code')" class="w-full" maxlength="10" /></UFormField>
          <UFormField label="Slug"><UInput v-model="form.slug" placeholder="Slug" class="w-full" /></UFormField>
          <UFormField :label="t('countries.flag')"><UInput v-model="form.flag" :placeholder="t('countries.flag')" class="w-full" /></UFormField>
    ...
    <AdminMappingSlideover v-model:open="mappingOpen" entity-type="COUNTRY" :entity-id="mappingEntityId" :entity-name="mappingEntityName" />
  </div>
</template>
...
const columns = [
  ...
  { accessorKey: 'name', header: t('common.name') }, { accessorKey: 'code', header: t('countries.code') },
  { accessorKey: 'slug', header: 'Slug' }, { accessorKey: 'flag', header: t('countries.flag') },
  { accessorKey: 'active', header: t('common.status') }
]

const { rows, total, totalPages, status, searchQuery, currentPage, pageSize, selectedIds, selectedRows, allSelected, someSelected, toggleAll, toggleRow, handleRefresh, filteredColumns, bulkPatch, bulkDelete } = useEntityList<any>('/api/countries', 'manage-countries', columns)

const formOpen = ref(false); const saving = ref(false); const editItem = ref<any>(null)
const form = reactive({ name: '', code: '', slug: '', flag: '', active: true })
const tr = reactive<Record<string, string>>({})

function openCreate() { editItem.value = null; Object.assign(form, { name: '', code: '', slug: '', flag: '', active: true }); Object.keys(tr).forEach(k => delete tr[k]); formOpen.value = true }
async function openEdit() {
  if (selectedRows.value.length !== 1) return; const row = selectedRows.value[0]; editItem.value = row
  Object.assign(form, { name: row.name ?? '', code: row.code ?? '', slug: row.slug ?? '', flag: row.flag ?? '', active: row.active ?? true })
  Object.keys(tr).forEach(k => delete tr[k])
  try { const res = await $fetch<{ data: any[] }>('/api/translations', { query: { entityType: 'COUNTRY', entityId: row.id } }); for (const t of res.data ?? []) tr[`${t.lang}:${t.field}`] = t.value } catch {}
  formOpen.value = true
}
async function handleSubmit() {
  if (!form.name.trim() || !form.code.trim()) { toast.add({ title: t('validation.required'), color: 'error' }); return }
  saving.value = true
  try {
    let id = editItem.value?.id
    if (id) { await $fetch(`/api/countries/${id}`, { method: 'PATCH', body: { ...form } }) }
    else { const c = await $fetch<{ id: number }>('/api/countries', { method: 'POST', body: { ...form } }); id = c.id }
    const trList = Object.entries(tr).filter(([, v]) => v?.trim()).map(([key, value]) => { const [lang, field] = key.split(':'); return { entityType: 'COUNTRY', entityId: id!, lang: lang!, field: field!, value: value.trim() } })
