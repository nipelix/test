    ...
    <UCard>
      ...
      <div class="space-y-4">
        ...
        <div class="flex flex-wrap items-center gap-1.5">
          <UButton icon="i-lucide-plus-circle" variant="outline" color="neutral" size="sm" @click="openAddCountry">
            {{ t('common.add') }}
          </UButton>
          <UButton icon="i-lucide-user-pen" variant="outline" color="neutral" size="sm" :disabled="selectedRows.length !== 1" @click="openEditCountry(selectedRows[0])">
            {{ t('common.edit') }}
          </UButton>
        ...
        <UCard>
          <template #header>
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-semibold">{{ editingCountry ? t('common.edit') : t('common.add') }} - {{ editingCountry?.name || t('dashboard.country') }}</h3>
          ...
          <template #footer>
            <div class="flex justify-end gap-2">
              ...
              <UButton color="primary" @click="saveCountry">{{ t('common.save') }}</UButton>
            </div>
          </template>
        </UCard>
      </template>
...
const columns = [
  ...
  { accessorKey: 'flag', header: '' },
  { accessorKey: 'name', header: t('common.name') },
  { accessorKey: 'code', header: 'Code' },
  { accessorKey: 'active', header: t('common.active') }
]

const { sorts, sortBy, sortDirection, addSort, removeSort, updateSortColumn, updateSortDirection, clearSorts, availableSortColumns, visibleColumnKeys, toggleColumn, filteredColumns, sortableColumns, toggleableColumns } = useTableStore('manage-countries', columns, 'name')
...
const { data: countriesData, refresh, status } = await useAsyncData(
  'manage-countries',
  () => $fetch<{ data: any[], total: number }>('/api/countries', { query: queryParams.value }),
  { watch: [queryParams] }
)

const tableData = computed(() => countriesData.value?.data ?? [])
const total = computed(() => countriesData.value?.total ?? 0)
...
async function bulkToggleActive(val: boolean) {
  try {
    await Promise.allSettled(selectedRows.value.map(c =>
      $fetch(`/api/countries/${c.id}`, { method: 'PATCH', body: { active: val } })
    ))
...
async function bulkDelete() {
  try {
    await Promise.allSettled(selectedRows.value.map(c =>
      $fetch(`/api/countries/${c.id}`, { method: 'DELETE' })
    ))
    selectedIds.value.clear()
    await refresh()
  } catch (err: any) {
...
const editForm = reactive({ name: '', code: '', active: true })

function openAddCountry() {
  editingCountry.value = null
  Object.assign(editForm, { name: '', code: '', active: true })
  showModal.value = true
}

function openEditCountry(country: any) {
  editingCountry.value = country
  Object.assign(editForm, { name: country.name, code: country.code, active: country.active })
  showModal.value = true
}
...
async function saveCountry() {
  try {
    if (editingCountry.value) {
      await $fetch(`/api/countries/${editingCountry.value.id}`, {
        method: 'PATCH',
        body: { name: editForm.name, code: editForm.code, active: editForm.active }
      })
    } else {
