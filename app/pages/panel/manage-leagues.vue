    ...
    <UCard>
      ...
      <div class="space-y-4">
        ...
        <div class="flex flex-wrap items-center gap-1.5">
          <UButton icon="i-lucide-user-pen" variant="outline" color="neutral" size="sm" :disabled="selectedRows.length !== 1" @click="openEditLeague(selectedRows[0])">
            {{ t('common.edit') }}
          </UButton>
        ...
        <UCard>
          <template #header>
            <div class="flex items-center justify-between">
              <h3 class="text-lg font-semibold">{{ t('common.edit') }} - {{ editingLeague?.name }}</h3>
          ...
          <template #footer>
            <div class="flex justify-end gap-2">
              ...
              <UButton color="primary" @click="saveLeague">{{ t('common.save') }}</UButton>
            </div>
          </template>
        </UCard>
      </template>
...
const columns = [
  ...
  { accessorKey: 'countryName', header: t('dashboard.country') },
...
const { sorts, sortBy, sortDirection, addSort, removeSort, updateSortColumn, updateSortDirection, clearSorts, availableSortColumns, visibleColumnKeys, toggleColumn, filteredColumns, sortableColumns, toggleableColumns } = useTableStore('manage-leagues', columns, 'sortOrder')
...
const { data: leaguesData, refresh, status } = await useAsyncData(
  'manage-leagues',
  () => $fetch<{ data: any[], total: number }>('/api/leagues', { query: queryParams.value }),
  { watch: [queryParams] }
)

const tableData = computed(() => leaguesData.value?.data ?? [])
const total = computed(() => leaguesData.value?.total ?? 0)
...
async function bulkToggleActive(val: boolean) {
  try {
    await Promise.allSettled(selectedRows.value.map(l =>
      $fetch(`/api/leagues/${l.id}`, { method: 'PATCH', body: { active: val } })
    ))
...
async function bulkDelete() {
  try {
    await Promise.allSettled(selectedRows.value.map(l =>
      $fetch(`/api/leagues/${l.id}`, { method: 'DELETE' })
    ))
    selectedIds.value.clear()
    await refresh()
  } catch (err: any) {
...
const editingLeague = ref<any>(null)
const editForm = reactive({ name: '', active: true })

function openEditLeague(league: any) {
  editingLeague.value = league
  Object.assign(editForm, { name: league.name, active: league.active })
  showModal.value = true
}

async function saveLeague() {
  if (!editingLeague.value) return
  try {
    await $fetch(`/api/leagues/${editingLeague.value.id}`, {
      method: 'PATCH',
