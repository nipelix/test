<template>
  <div class="space-y-3">
    ...
    <div class="flex items-center justify-between gap-4">
      <UInput v-model="searchModel" icon="i-lucide-search" :placeholder="searchPlaceholder" class="max-w-xs" />
      <div class="flex items-center gap-2">
        ...
        <AdminTableSortPopover
          ...
          @remove-sort="(i) => $emit('remove-sort', i)"
          @update-sort-column="(i, k) => $emit('update-sort-column', i, k)"
          @update-sort-direction="(i, d) => $emit('update-sort-direction', i, d)"
          @clear-sorts="$emit('clear-sorts')"
        />
        <AdminTableViewPopover
          :toggleable-columns="toggleableColumns"
          :visible-columns="visibleColumns"
          @toggle-column="(k) => $emit('toggle-column', k)"
        />
      </div>
    </div>
  </div>
...
const searchModel = computed({
  get: () => props.search,
  set: (val) => {
    // Will be handled via v-model by parent
  }
})
</script>
